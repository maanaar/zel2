...
import { ChevronLeft, ChevronDown, ChevronRight, Filter, Star, Users, Clock } from 'lucide-react';
      ...
      {selectedService ? (
        <div>
          <div className="mb-4 flex justify-between items-center">
            ...
            <button className="flex items-center text-purple-700 border border-purple-300 rounded px-3 py-1 hover:bg-purple-50" onClick={() => setShowPartnerFilter(prev => !prev)}>
          ...
          {showPartnerFilter && (
            <div className="bg-white border border-gray-200 rounded-md p-4 mb-4 shadow-sm">
              <p className="font-medium mb-2 text-gray-700">Filter by Partner:</p>
              <div className="flex flex-wrap gap-4">
                {allPartners.map((partner, idx) => (
                  <label key={idx} className="flex items-center gap-2 text-sm text-gray-700">
                    <input
                      type="checkbox"
                      checked={selectedPartnerFilters.includes(partner)}
                      onChange={() => {
                        setSelectedPartnerFilters(prev =>
          ...
          <div className="flex flex-col gap-8">
            {selectedService.partners
              .filter(partner => {
                if (!selectedPartnerFilters.length) return true;
                return selectedPartnerFilters.includes(partner);
              })
              .map((partner, index) => {
                  ...
                  <div key={index} className="flex flex-col lg:flex-row bg-white rounded-xl shadow p-4 gap-6">
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col justify-between pt-4 lg:pt-0">
                      <div>
                        ...
                        <div className="text-sm text-purple-600 underline cursor-pointer hover:text-purple-800 mb-2">
                          more reviews
                        </div>
